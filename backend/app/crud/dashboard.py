...
def get_dashboard_summary(db: Session, user_id: int, month: str | None = None):
    ...
    ).filter(Income.user_id == user_id, Income.amount > 0, Income.date >= start, Income.date < end).group_by(income_year, income_month).order_by(income_year, income_month).all()
    monthly_expense = db.query(
        expense_year.label("year"), expense_month.label("month"), func.sum(Expense.amount).label("amount")
    ).filter(Expense.user_id == user_id, Expense.date >= start, Expense.date < end).group_by(expense_year, expense_month).order_by(expense_year, expense_month).all()
    ...
    expenses = db.query(Expense).filter(Expense.user_id == user_id, Expense.date >= start, Expense.date < end).order_by(Expense.date.desc()).limit(5).all()
    incomes = db.query(Income).filter(Income.user_id == user_id, Income.amount > 0, Income.date >= start, Income.date < end).order_by(Income.date.desc()).limit(5).all()
    transactions = [
