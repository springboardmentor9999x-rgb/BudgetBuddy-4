...
export default function Reports(){
 ...
 const download=async format=>{try{setError("");setExporting(format);await downloadReport(format,period);setReport(current=>({...current,export_allowance:{...current.export_allowance,remaining:current.export_allowance?.remaining==null?null:Math.max(0,current.export_allowance.remaining-1)}}));}catch(err){let message="The statement could not be downloaded. Please try again.";try{const data=err.response?.data;const details=data instanceof Blob?JSON.parse(await data.text()):data;if(typeof details?.detail==="string")message=details.detail;}catch{/* Use the download error fallback. */}setError(message);}finally{setExporting("");}};
 if(error&&!report)return <div className="reports-message error">{error}</div>;if(!report)return <div className="reports-message">Preparing your statement…</div>;
 return <div className="reports-page statement-page">
  <header className="statement-toolbar"><div><p>Account statement</p><h1>Monthly statement</h1><span>A clear, bank-style record of every credit and debit.</span></div><div className="reports-actions"><label>Month<select value={period.month} onChange={e=>setPeriod(p=>({...p,month:+e.target.value}))}>{Array.from({length:12},(_,i)=><option key={i} value={i+1}>{new Date(2000,i).toLocaleString("en",{month:"long"})}</option>)}</select></label><label>Year<select value={period.year} onChange={e=>setPeriod(p=>({...p,year:+e.target.value}))}>{[period.year-1,period.year,period.year+1].map(y=><option key={y}>{y}</option>)}</select></label><button onClick={()=>download("pdf")} disabled={!!exporting||report.export_allowance?.remaining===0}><FaFilePdf/>{exporting==="pdf"?"Preparing…":"PDF statement"}</button><button onClick={()=>download("excel")} disabled={!!exporting||report.export_allowance?.remaining===0}><FaFileExcel/>{exporting==="excel"?"Preparing…":"Excel statement"}</button></div></header>
  {error&&<p className="reports-message error" role="alert">{error}</p>}
  {report.export_allowance?.limit!=null&&<p role="status">Free plan: {report.export_allowance.remaining} of {report.export_allowance.limit} lifetime exports remaining (PDF and Excel combined). {report.export_allowance.remaining===0&&"Upgrade to Premium for unlimited exports."}</p>}
