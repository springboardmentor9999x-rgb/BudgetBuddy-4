...
function Settings() {
  ...
  const save = () => { localStorage.setItem("budgetbuddy-settings", JSON.stringify(settings)); setSaved(true); };
  const logout = () => { localStorage.removeItem("token"); navigate("/login", { replace: true }); };

  return <div className="settings-page">
    <header className="settings-heading">
      <div><p>Personalize your workspace</p><h1>Settings</h1><span>Manage how BudgetBuddy works for you.</span></div>
    ...
    <section className="settings-section profile-section">
      ...
      <div className="settings-panel">
        <div className="account-row"><div className="settings-avatar">V</div><div><strong>Personal account</strong><p>Your profile details are managed securely.</p></div></div>
        <Link className="settings-link-row" to="/forgot-password"><span><FaLock /> Change password</span><FaChevronRight /></Link>
      </div>
    </section>
    ...
    <section className="settings-section">
      ...
      <div className="settings-panel settings-controls">
        ...
        <label>Appearance<select value={settings.theme} onChange={(e) => update("theme", e.target.value)}><option value="system">Use device setting</option><option value="light">Light</option></select></label>
