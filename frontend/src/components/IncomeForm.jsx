...
function IncomeForm({ accounts, onAdd }) {
  ...
  const submit = async (event) => { event.preventDefault(); setError(""); const account = accounts.find((item) => item.id === accountId); if (!account) { setError("Please select a bank account."); return; } try { setSaving(true); await onAdd({ source: source.trim(), amount: Number(amount), description: description.trim(), bank_account: `${account.bankName} •••• ${account.lastFour}` }); setSource(""); setAmount(""); setDescription(""); setAccountId(""); } catch (requestError) { setError(requestError.response?.data?.detail || "Unable to add income."); } finally { setSaving(false); } };
